...
Though elucidating the causes of species' ranges has been a central question in ecology and evolutionary biology for decades (MacArthur 1984; Brown et al. 1996; Holt 2003; Sexton et al. 2009), the question has not been answered for a single species. \textbf{Approaches commonly used to study geographic distributions, species distribution models, typically assume species ranges are determined primarily by climate. Add something about niches? Scale of climate versus competition effects?}. However, Dobzhansky (1950) and MacArthur (1972) both suggested that species' ranges are limited by abiotic stress at high latitudes and/or elevations, and limited by biotic stress (competition) at low latitudes and/or elevations. Their relative contributions are not clear (Brooker et al. 2007; Sexton et al. 2009), including for tree species (Ettinger et al. 2011; Ettinger \& HilleRisLambers 2013; HilleRisLambers et al. 2013). 
...
\emph{Climate + competition, no interactions}: Adding competition to the model improved the maps (Fig. \ref{fig:map_cc}), but not in the way we would predict based on our hypothesis. Based on our hypothesis, competition would be limiting at upper elevations, so adding competition to the model woudl result in better predictions of the upper-elevation limit. However, the model still predicted high $\lambda$ values at high elevations. It did, however, result in better predictions at low elevations. It predicted low $\lambda$ values in the southwestern desert, although it did predict high lambda $\lambda$ in what we think is grassland in the southeastern region. It therefore seems that a lot of the improvement is due to better predictions of climate effects when competition is added to the model, most notably by improving the prediction of the relationship between temperature and regeneration (Fig. \ref{fig:recr_cc}c vs Fig. \ref{fig:recr_c}b).

\emph{Climate + competition, with interactions}: To the eye, adding interactions to the models slightly improved preditions (Fig. \ref{fig:map_i}). It did not predict such high lambda values in the southeastern grassland region. However the map looked fairly similar to the map without interactions, so this is a place where Florian's residual analysis would be helpful for interpreting the results.
...
<<echo=FALSE>>=
ncuts=30
chopsize_dia<-cut(grdata.scaled$PREVDIA,ncuts)
chopsize_ba<-cut(grdata.scaled$BALIVE,ncuts)
chopsize_PPT<-cut(grdata.scaled$PPT_yr_norm,ncuts)
chopsize_T<-cut(grdata.scaled$T_yr_norm,ncuts)
grow_binned_dia<-as.vector(sapply(split(grdata.scaled$DIA_INCR,chopsize_dia),mean,na.rm=T))
dia_binned<-as.vector(sapply(split(grdata.scaled$PREVDIA,chopsize_dia),mean,na.rm=T))
grow_binned_ba<-as.vector(sapply(split(grdata.scaled$DIA_INCR,chopsize_ba),mean,na.rm=T))
ba_binned<-as.vector(sapply(split(grdata.scaled$BALIVE,chopsize_ba),mean,na.rm=T))
grow_binned_PPT<-as.vector(sapply(split(grdata.scaled$DIA_INCR,chopsize_PPT),mean,na.rm=T))
PPT_binned<-as.vector(sapply(split(grdata.scaled$PPT_yr_norm,chopsize_PPT),mean,na.rm=T))
grow_binned_T<-as.vector(sapply(split(grdata.scaled$DIA_INCR,chopsize_T),mean,na.rm=T))
T_binned<-as.vector(sapply(split(grdata.scaled$T_yr_norm,chopsize_T),mean,na.rm=T))
...
g_fun<-function(dia,ba,ppt,t,model,clampba=F,clampt=F){
  data=data.frame(PREVDIA=dia,BALIVE = ifelse(clampba==T & ba >1.5, 1.5, ba),
  PPT_yr_norm = ppt, T_yr_norm = ifelse(clampt==T & t >1, 1, t))
  return(predict(model, newdata = data, re.form = NA))
}
means<-c(mean(grdata.scaled$PREVDIA,na.rm=T),mean(grdata.scaled$BALIVE,na.rm=T),
         mean(grdata.scaled$PPT_yr_norm,na.rm=T),mean(grdata.scaled$T_yr_norm,na.rm=T))
seq<-data.frame(dia=seq(1.5*min(grdata.scaled$PREVDIA),1.5*max(grdata.scaled$PREVDIA),length=50),
                ba=seq(1.5*min(grdata.scaled$BALIVE),1.5*max(grdata.scaled$BALIVE),length=50),
                ppt=seq(1.5*min(grdata.scaled$PPT_yr_norm),
                        1.5*max(grdata.scaled$PPT_yr_norm),length=50),
                t=seq(1.5*min(grdata.scaled$T_yr_norm),1.5*max(grdata.scaled$T_yr_norm),length=50))
...
chopsize_PPT<-cut(survData3.scaled$PPT_yr_norm,ncuts)
...
chopsize_T<-cut(survData3.scaled$T_yr_norm,ncuts)
...
surv_binned_dia<-as.vector(sapply(split(survData3.scaled$mort,chopsize_dia),mean,na.rm=T))
dia_binned<-as.vector(sapply(split(survData3.scaled$PREVDIA,chopsize_dia),mean,na.rm=T))
surv_binned_ba<-as.vector(sapply(split(survData3.scaled$mort,chopsize_ba),mean,na.rm=T))
ba_binned<-as.vector(sapply(split(survData3.scaled$BALIVE,chopsize_ba),mean,na.rm=T))
surv_binned_PPT<-as.vector(sapply(split(survData3.scaled$mort,chopsize_PPT),mean,na.rm=T))
PPT_binned<-as.vector(sapply(split(survData3.scaled$PPT_yr_norm,chopsize_PPT),mean,na.rm=T))
surv_binned_PPTc<-as.vector(sapply(split(survData3.scaled$mort,chopsize_PPTc),mean,na.rm=T))
PPTc_binned<-as.vector(sapply(split(survData3.scaled$PPT_c_norm,chopsize_PPTc),mean,na.rm=T))
surv_binned_PPTpf<-as.vector(sapply(split(survData3.scaled$mort,chopsize_PPTpf),mean,na.rm=T))
PPTpf_binned<-as.vector(sapply(split(survData3.scaled$PPT_pf_norm,chopsize_PPTpf),mean,na.rm=T))
surv_binned_PPTfs<-as.vector(sapply(split(survData3.scaled$mort,chopsize_PPTfs),mean,na.rm=T))
PPTfs_binned<-as.vector(sapply(split(survData3.scaled$PPT_fs_norm,chopsize_PPTfs),mean,na.rm=T))
surv_binned_PPTm<-as.vector(sapply(split(survData3.scaled$mort,chopsize_PPTm),mean,na.rm=T))
PPTm_binned<-as.vector(sapply(split(survData3.scaled$PPT_m_norm,chopsize_PPTm),mean,na.rm=T))
surv_binned_T<-as.vector(sapply(split(survData3.scaled$mort,chopsize_T),mean,na.rm=T))
T_binned<-as.vector(sapply(split(survData3.scaled$T_yr_norm,chopsize_T),mean,na.rm=T))
surv_binned_Tc<-as.vector(sapply(split(survData3.scaled$mort,chopsize_Tc),mean,na.rm=T))
Tc_binned<-as.vector(sapply(split(survData3.scaled$T_c_norm,chopsize_Tc),mean,na.rm=T))
surv_binned_Tpf<-as.vector(sapply(split(survData3.scaled$mort,chopsize_Tpf),mean,na.rm=T))
Tpf_binned<-as.vector(sapply(split(survData3.scaled$T_pf_norm,chopsize_Tpf),mean,na.rm=T))
surv_binned_Tfs<-as.vector(sapply(split(survData3.scaled$mort,chopsize_Tfs),mean,na.rm=T))
Tfs_binned<-as.vector(sapply(split(survData3.scaled$T_fs_norm,chopsize_Tfs),mean,na.rm=T))
surv_binned_Tm<-as.vector(sapply(split(survData3.scaled$mort,chopsize_Tm),mean,na.rm=T))
Tm_binned<-as.vector(sapply(split(survData3.scaled$T_m_norm,chopsize_Tm),mean,na.rm=T))
...
s_fun<-function(dia,ba,ppt,t,ci,model,clampt=F){
  data=data.frame(PREVDIA=dia,BALIVE=ba,PPT_yr_norm = ppt, 
                  T_yr_norm = ifelse(clampt==T & t >2, 2, t),CENSUS_INTERVAL=ci)
  return(predict(model, newdata = data, type = "response", re.form = NA))
}
means<-c(mean(survData3.scaled$PREVDIA,na.rm=T),mean(survData3.scaled$BALIVE,na.rm=T),
         mean(survData3.scaled$PPT_yr_norm,na.rm=T),mean(survData3.scaled$T_yr_norm,na.rm=T),
         mean(survData3.scaled$CENSUS_INTERVAL,na.rm=T))
seq<-data.frame(dia=seq(1.5*min(survData3.scaled$PREVDIA),1.5*max(survData3.scaled$PREVDIA),length=50),
                ba=seq(1.5*min(survData3.scaled$BALIVE),1.5*max(survData3.scaled$BALIVE),length=50),
                ppt=seq(1.5*min(survData3.scaled$PPT_yr_norm),
                        1.5*max(survData3.scaled$PPT_yr_norm),length=50),
                t=seq(1.5*min(survData3.scaled$T_yr_norm),
                      1.5*max(survData3.scaled$T_yr_norm),length=50))
...
<<echo=FALSE>>=
ncuts=30
chopsize_ba<-cut(rdata.scaled$BALIVE,ncuts)
chopsize_PPT<-cut(rdata.scaled$PPT_yr_norm,ncuts)
chopsize_T<-cut(rdata.scaled$T_yr_norm,ncuts)
recr_binned_ba<-as.vector(sapply(split(rdata.scaled$recruits1,chopsize_ba),mean,na.rm=T))
ba_binned<-as.vector(sapply(split(rdata.scaled$BALIVE,chopsize_ba),mean,na.rm=T))
recr_binned_PPT<-as.vector(sapply(split(rdata.scaled$recruits1,chopsize_PPT),mean,na.rm=T))
PPT_binned<-as.vector(sapply(split(rdata.scaled$PPT_yr_norm,chopsize_PPT),mean,na.rm=T))
recr_binned_T<-as.vector(sapply(split(rdata.scaled$recruits1,chopsize_T),mean,na.rm=T))
T_binned<-as.vector(sapply(split(rdata.scaled$T_yr_norm,chopsize_T),mean,na.rm=T))
...
r_fun<-function(ba,ppt,t,pa,ci,model,clampba=F){
  data=data.frame(BALIVE = ifelse(clampba==T & ba >2.5, 2.5, ba),
  PPT_yr_norm = ppt, T_yr_norm = t,PIEDadults1=pa,CENSUS_INTERVAL=ci)
  return(predict(model, newdata = data, type = "response"))
}
means<-c(mean(rdata.scaled$BALIVE,na.rm=T),mean(rdata.scaled$PPT_yr_norm,na.rm=T),
         mean(rdata.scaled$T_yr_norm,na.rm=T),mean(rdata.scaled$PIEDadults1),
         mean(rdata.scaled$CENSUS_INTERVAL))
seq<-data.frame(ba=seq(1.5*min(rdata.scaled$BALIVE),1.5*max(rdata.scaled$BALIVE),length=50),
                ppt=seq(1.5*min(rdata.scaled$PPT_yr_norm),1.5*max(rdata.scaled$PPT_yr_norm),length=50),
                t=seq(1.5*min(rdata.scaled$T_yr_norm),1.5*max(rdata.scaled$T_yr_norm),length=50))
...
for (i in 1:ncol(predictors)) {
  ...
  for (j in 1:nrow(predictorVals)) {
    predictorVals[j, "lambda"] <- Re(eigen(ipm_fun(min=min.size, max=max.size, n=n_dim, 
                                                   ...
                                                   rmodel=rmodel.clim, 
                                                   gSD=growSD.clim,
                                          data=predictorVals[j,],g.t.clamp=T))$values[1])
  }
...
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=PREVDIA))+
  geom_rect(aes(xmin=min(grplot_data_clim$dia),xmax=min(grdata.scaled$PREVDIA),ymin=-1.5,ymax=0.1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_clim$dia),xmin=max(grdata.scaled$PREVDIA),ymin=-1.5,ymax=0.1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=-1.6),size=0.1)+
  geom_point(data=g_binned,aes(x=PREVDIA,y=grow_dia))+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=PPT_yr_norm))+
  geom_rect(aes(xmin=min(grplot_data_clim$ppt),xmax=min(grdata.scaled$PPT_yr_norm),
                ymin=0.015,ymax=0.11),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_clim$ppt),xmin=max(grdata.scaled$PPT_yr_norm),
                ymin=0.015,ymax=0.11),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=0.01),size=0.1)+
  geom_point(data=g_binned,aes(x=PPT,y=grow_PPT))+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
g_clim_t<-ggplot(data=grdata.scaled,aes(x=T_yr_norm))+
  geom_rect(aes(xmin=min(grplot_data_clim$t),xmax=min(grdata.scaled$T_yr_norm),
                ymin=-0.002,ymax=0.085),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_clim$t),xmin=max(grdata.scaled$T_yr_norm),
                ymin=-0.002,ymax=0.085),fill="grey80",col="grey80",alpha=0.1)+
...
g_clim_t + theme(legend.position = "none")
@
  \end{minipage}%
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=FALSE>>=
...
\begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=PREVDIA,y=mort_dia))+
  geom_rect(aes(xmin=min(splot_data_clim$dia),xmax=min(survData3.scaled$PREVDIA),ymin=-0.02,ymax=0.7),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_clim$dia),xmin=max(survData3.scaled$PREVDIA),ymin=-0.02,ymax=0.7),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=PREVDIA,y=-0.05),size=0.1)+
  geom_point()+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=PPT,y=mort_PPT))+
  geom_rect(aes(xmin=min(splot_data_clim$ppt),xmax=min(survData3.scaled$PPT_yr_norm),
                ymin=-0.02,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_clim$ppt),xmin=max(survData3.scaled$PPT_yr_norm),
                ymin=-0.02,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=PPT_yr_norm,y=-0.05),size=0.1)+
  geom_point()+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=T,y=mort_T))+
  geom_rect(aes(xmin=min(splot_data_clim$t),xmax=min(survData3.scaled$T_yr_norm),
                ymin=-0.02,ymax=0.55),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_clim$t),xmin=max(survData3.scaled$T_yr_norm),
                ymin=-0.02,ymax=0.55),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=T_yr_norm,y=-0.05),size=0.1)+
  geom_point()+
...
\begin{figure}[H]
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=rdata.scaled,aes(x=PPT_yr_norm))+
  geom_rect(aes(xmin=min(rplot_data_clim$ppt),xmax=min(rdata.scaled$PPT_yr_norm),
                ymin=-0.05,ymax=2),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(rplot_data_clim$ppt),xmin=max(rdata.scaled$PPT_yr_norm),
                ymin=-0.05,ymax=2),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=r_binned,aes(x=PPT,y=recr_PPT))+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=rdata.scaled,aes(x=T_yr_norm))+
  geom_rect(aes(xmin=min(rplot_data_clim$t),xmax=min(rdata.scaled$T_yr_norm),
                ymin=-0.05,ymax=2.7),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(rplot_data_clim$t),xmin=max(rdata.scaled$T_yr_norm),
                ymin=-0.05,ymax=2.7),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=r_binned,aes(x=T,y=recr_T))+
...
ggplot(data = predictorDFs_clim[[2]], aes(x = PPT_yr, y = lambda)) + 
  geom_rect(aes(xmin=min(predictorDFs_clim[[2]]$PPT_yr),xmax=min(FIA$PPT_yr_norm),ymin=0.99,ymax=1.15),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(predictorDFs_clim[[2]]$PPT_yr),xmin=max(FIA$PPT_yr_norm),ymin=0.99,ymax=1.15),
            fill="grey80",col="grey80",alpha=0.1) +
...
ggplot(data = predictorDFs_clim[[3]], aes(x = T_yr, y = lambda)) +
  geom_rect(aes(xmin=min(predictorDFs_clim[[3]]$T_yr),xmax=min(FIA$T_yr_norm),ymin=0.99,ymax=1.55),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(predictorDFs_clim[[3]]$T_yr),xmin=max(FIA$T_yr_norm),ymin=0.99,ymax=1.55),
            fill="grey80",col="grey80",alpha=0.1) +
  geom_point() + geom_line() +
  geom_hline(yintercept=1,linetype="dashed")+
...
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=PREVDIA))+
  geom_rect(aes(xmin=min(grplot_data_climcomp$dia),xmax=min(grdata.scaled$PREVDIA),ymin=-1.5,ymax=0.1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_climcomp$dia),xmin=max(grdata.scaled$PREVDIA),ymin=-1.5,ymax=0.1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=-1.6),size=0.1)+
  geom_point(data=g_binned,aes(x=PREVDIA,y=grow_dia))+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=BALIVE))+
  geom_rect(aes(xmin=min(grplot_data_climcomp$ba),xmax=min(grdata.scaled$BALIVE),
                ymin=-0.002,ymax=0.2),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_climcomp$ba),xmin=max(grdata.scaled$BALIVE),
                ymin=-0.002,ymax=0.2),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=-0.005),size=0.1)+
  geom_point(data=g_binned,aes(x=BALIVE,y=grow_ba))+
...
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=PPT_yr_norm))+
  geom_rect(aes(xmin=min(grplot_data_climcomp$ppt),xmax=min(grdata.scaled$PPT_yr_norm),
                ymin=0.015,ymax=0.09),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_climcomp$ppt),xmin=max(grdata.scaled$PPT_yr_norm),
                ymin=0.015,ymax=0.09),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=0.01),size=0.1)+
  geom_point(data=g_binned,aes(x=PPT,y=grow_PPT))+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=T_yr_norm))+
  geom_rect(aes(xmin=min(grplot_data_climcomp$t),xmax=min(grdata.scaled$T_yr_norm),
                ymin=-0.002,ymax=0.07),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_climcomp$t),xmin=max(grdata.scaled$T_yr_norm),
                ymin=-0.002,ymax=0.07),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=-0.005),size=0.1)+
  geom_point(data=g_binned,aes(x=T,y=grow_T))+
...
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=PREVDIA,y=mort_dia))+
  geom_rect(aes(xmin=min(splot_data_climcomp$dia),xmax=min(survData3.scaled$PREVDIA),
                ymin=-0.02,ymax=0.7),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_climcomp$dia),xmin=max(survData3.scaled$PREVDIA),
                ymin=-0.02,ymax=0.7),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=PREVDIA,y=-0.05),size=0.1)+
  geom_point()+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=BALIVE,y=mort_ba))+
  geom_rect(aes(xmin=min(splot_data_climcomp$ba),xmax=min(survData3.scaled$BALIVE),ymin=-0.02,ymax=1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_climcomp$ba),xmin=max(survData3.scaled$BALIVE),ymin=-0.02,ymax=1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=BALIVE,y=-0.05),size=0.1)+
  geom_point()+
...
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=PPT,y=mort_PPT))+
  geom_rect(aes(xmin=min(splot_data_climcomp$ppt),xmax=min(survData3.scaled$PPT_yr_norm),
                ymin=-0.02,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_climcomp$ppt),xmin=max(survData3.scaled$PPT_yr_norm),
                ymin=-0.02,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=PPT_yr_norm,y=-0.05),size=0.1)+
  geom_point()+
...
@
  ...
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
s_climcomp_t<-ggplot(data=s_binned,aes(x=T,y=mort_T))+
  geom_rect(aes(xmin=min(splot_data_climcomp$t),xmax=min(survData3.scaled$T_yr_norm),
                ymin=-0.02,ymax=0.55),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_climcomp$t),xmin=max(survData3.scaled$T_yr_norm),
                ymin=-0.02,ymax=0.55),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=T_yr_norm,y=-0.05),size=0.1)+
  geom_point()+
  ...
  theme(legend.key.width=unit(2,"cm"),legend.text=element_text(size=24),
        legend.key.size = unit(3, 'lines'))
s_climcomp_t + theme(legend.position = "none")
@
  \end{minipage}%
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=FALSE>>=
...
\begin{figure}[H]
  \begin{minipage}[b]{.5\linewidth}
<<fig=TRUE,echo=false>>=
ggplot(data=rdata.scaled,aes(x=BALIVE))+
  geom_rect(aes(xmin=min(rplot_data_climcomp$ba),xmax=min(rdata.scaled$BALIVE),
                ymin=-0.05,ymax=7.5),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(rplot_data_climcomp$ba),xmin=max(rdata.scaled$BALIVE),
                ymin=-0.05,ymax=7.5),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=r_binned,aes(x=BALIVE,y=recr_ba))+
...
<<fig=TRUE,echo=false>>=
ggplot(data=rdata.scaled,aes(x=PPT_yr_norm))+
  geom_rect(aes(xmin=min(rplot_data_climcomp$ppt),xmax=min(rdata.scaled$PPT_yr_norm),
                ymin=-0.05,ymax=2),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(rplot_data_climcomp$ppt),xmin=max(rdata.scaled$PPT_yr_norm),
                ymin=-0.05,ymax=2),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=r_binned,aes(x=PPT,y=recr_PPT))+
  geom_point(aes(y=-0.15),size=0.1)+
...
<<fig=TRUE,echo=false>>=
ggplot(data=rdata.scaled,aes(x=T_yr_norm))+
  geom_rect(aes(xmin=min(rplot_data_climcomp$t),xmax=min(rdata.scaled$T_yr_norm),
                ymin=-0.05,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(rplot_data_climcomp$t),xmin=max(rdata.scaled$T_yr_norm),
                ymin=-0.05,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=r_binned,aes(x=T,y=recr_T))+
  geom_point(aes(y=-0.1),size=0.1)+
...
ggplot(data = predictorDFs_climcomp[[1]], aes(x = BALIVE, y = lambda)) +   
  geom_rect(aes(xmin=min(predictorDFs_climcomp[[1]]$BALIVE),xmax=min(FIA$BALIVE),ymin=0.82,ymax=1.15),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(predictorDFs_climcomp[[1]]$BALIVE),xmin=max(FIA$BALIVE),ymin=0.82,ymax=1.15),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_point() + geom_line() +
  geom_hline(yintercept=1,linetype="dashed")+
...
ggplot(data = predictorDFs_climcomp[[2]], aes(x = PPT_yr, y = lambda)) + 
  geom_rect(aes(xmin=min(predictorDFs_climcomp[[2]]$PPT_yr),xmax=min(FIA$PPT_yr_norm),
                ymin=0.99,ymax=1.05),fill="grey80",col="grey80",alpha=0.1)+
...
ggplot(data = predictorDFs_climcomp[[3]], aes(x = T_yr, y = lambda)) + 
  geom_rect(aes(xmin=min(predictorDFs_climcomp[[3]]$T_yr),xmax=min(FIA$T_yr_norm),
                ymin=0.994,ymax=1.02),fill="grey80",col="grey80",alpha=0.1)+
...
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=PREVDIA))+
  geom_rect(aes(xmin=min(grplot_data_int$dia),xmax=min(grdata.scaled$PREVDIA),ymin=-1.5,ymax=0.1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_int$dia),xmin=max(grdata.scaled$PREVDIA),ymin=-1.5,ymax=0.1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=-1.6),size=0.1)+
  geom_point(data=g_binned,aes(x=PREVDIA,y=grow_dia))+
...
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=BALIVE))+
  geom_rect(aes(xmin=min(grplot_data_int$ba),xmax=min(grdata.scaled$BALIVE),
                ymin=-0.002,ymax=0.2),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_int$ba),xmin=max(grdata.scaled$BALIVE),
                ymin=-0.002,ymax=0.2),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=-0.005),size=0.1)+
  geom_point(data=g_binned,aes(x=BALIVE,y=grow_ba))+
...
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=PPT_yr_norm))+
  geom_rect(aes(xmin=min(grplot_data_int$ppt),xmax=min(grdata.scaled$PPT_yr_norm),
                ymin=0.015,ymax=0.09),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_int$ppt),xmin=max(grdata.scaled$PPT_yr_norm),
                ymin=0.015,ymax=0.09),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=0.01),size=0.1)+
  geom_point(data=g_binned,aes(x=PPT,y=grow_PPT))+
...
<<fig=TRUE,echo=false>>=
ggplot(data=grdata.scaled,aes(x=T_yr_norm))+
  geom_rect(aes(xmin=min(grplot_data_int$t),xmax=min(grdata.scaled$T_yr_norm),
                ymin=-0.002,ymax=0.07),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(grplot_data_int$t),xmin=max(grdata.scaled$T_yr_norm),
                ymin=-0.002,ymax=0.07),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(aes(y=-0.005),size=0.1)+
  geom_point(data=g_binned,aes(x=T,y=grow_T))+
...
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=PREVDIA,y=mort_dia))+
  geom_rect(aes(xmin=min(splot_data_int$dia),xmax=min(survData3.scaled$PREVDIA),
                ymin=-0.02,ymax=0.7),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_int$dia),xmin=max(survData3.scaled$PREVDIA),
                ymin=-0.02,ymax=0.7),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=PREVDIA,y=-0.05),size=0.1)+
  geom_point()+
...
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=BALIVE,y=mort_ba))+
  geom_rect(aes(xmin=min(splot_data_int$ba),xmax=min(survData3.scaled$BALIVE),ymin=-0.02,ymax=1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_int$ba),xmin=max(survData3.scaled$BALIVE),ymin=-0.02,ymax=1),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=BALIVE,y=-0.08),size=0.1)+
  geom_point()+
...
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=PPT,y=mort_PPT))+
  geom_rect(aes(xmin=min(splot_data_int$ppt),xmax=min(survData3.scaled$PPT_yr_norm),
                ymin=-0.02,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_int$ppt),xmin=max(survData3.scaled$PPT_yr_norm),
                ymin=-0.02,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=PPT_yr_norm,y=-0.08),size=0.1)+
  geom_point()+
...
<<fig=TRUE,echo=false>>=
ggplot(data=s_binned,aes(x=T,y=mort_T))+
  geom_rect(aes(xmin=min(splot_data_int$t),xmax=min(survData3.scaled$T_yr_norm),
                ymin=-0.02,ymax=0.55),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(splot_data_int$t),xmin=max(survData3.scaled$T_yr_norm),
                ymin=-0.02,ymax=0.55),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=survData3.scaled,aes(x=T_yr_norm,y=-0.07),size=0.1)+
  geom_point()+
...
<<fig=TRUE,echo=false>>=
ggplot(data=rdata.scaled,aes(x=BALIVE))+
  geom_rect(aes(xmin=min(rplot_data_int$ba),xmax=min(rdata.scaled$BALIVE),
                ymin=-0.4,ymax=20),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(rplot_data_int$ba),xmin=max(rdata.scaled$BALIVE),
                ymin=-0.4,ymax=20),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=r_binned,aes(x=BALIVE,y=recr_ba))+
...
<<fig=TRUE,echo=false>>=
ggplot(data=rdata.scaled,aes(x=PPT_yr_norm))+
  geom_rect(aes(xmin=min(rplot_data_int$ppt),xmax=min(rdata.scaled$PPT_yr_norm),
                ymin=-0.05,ymax=2),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(rplot_data_int$ppt),xmin=max(rdata.scaled$PPT_yr_norm),
                ymin=-0.05,ymax=2),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=r_binned,aes(x=PPT,y=recr_PPT))+
  geom_point(aes(y=-0.15),size=0.1)+
...
<<fig=TRUE,echo=false>>=
ggplot(data=rdata.scaled,aes(x=T_yr_norm))+
  geom_rect(aes(xmin=min(rplot_data_int$t),xmax=min(rdata.scaled$T_yr_norm),
                ymin=-0.05,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(rplot_data_int$t),xmin=max(rdata.scaled$T_yr_norm),
                ymin=-0.05,ymax=0.8),fill="grey80",col="grey80",alpha=0.1)+
  geom_point(data=r_binned,aes(x=T,y=recr_T))+
  geom_point(aes(y=-0.1),size=0.1)+
...
ggplot(data = predictorDFs_int[[1]], aes(x = BALIVE, y = lambda)) + 
  geom_rect(aes(xmin=min(predictorDFs_int[[1]]$BALIVE),xmax=min(FIA$BALIVE),ymin=0.9,ymax=1.13),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(predictorDFs_int[[1]]$BALIVE),xmin=max(FIA$BALIVE),ymin=0.9,ymax=1.13),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_point() + geom_line() +
  geom_hline(yintercept=1,linetype="dashed")+
  xlab("Plot basal area") + ylab("Lambda")
...
ggplot(data = predictorDFs_int[[2]], aes(x = PPT_yr, y = lambda)) + 
  geom_rect(aes(xmin=min(predictorDFs_int[[2]]$PPT_yr),xmax=min(FIA$PPT_yr_norm),ymin=0.99,ymax=1.08),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(predictorDFs_int[[2]]$PPT_yr),xmin=max(FIA$PPT_yr_norm),ymin=0.99,ymax=1.08),
            fill="grey80",col="grey80",alpha=0.1) +
  geom_point() + geom_line() +
  geom_hline(yintercept=1,linetype="dashed")+
  xlab("MAP") + ylab("Lambda")
...
ggplot(data = predictorDFs_int[[3]], aes(x = T_yr, y = lambda)) + 
  geom_rect(aes(xmin=min(predictorDFs_int[[3]]$T_yr),xmax=min(FIA$T_yr_norm),ymin=0.994,ymax=1.02),
            fill="grey80",col="grey80",alpha=0.1)+
  geom_rect(aes(xmax=max(predictorDFs_int[[3]]$T_yr),xmin=max(FIA$T_yr_norm),ymin=0.994,ymax=1.02),
            fill="grey80",col="grey80",alpha=0.1) +
  geom_point() + geom_line() +
  geom_hline(yintercept=1,linetype="dashed")+
  xlab("MAT") + ylab("Lambda")
