...
pres_binned<-data.frame(count_lam=c(count_binned_lam_c,count_binned_lam_ci,count_binned_lam_cc,count_binned_lam_i),
                        lam=c(lam_c_binned,lam_ci_binned,lam_cc_binned,lam_i_binned),
                        pres=c(pres_c_binned,pres_ci_binned,pres_cc_binned,pres_i_binned),
                        pred=c(invlogit(predict(pa_c,newdata=data.frame(lambda_c=lam_c_binned))),
                               invlogit(predict(pa_ci,newdata=data.frame(lambda_ci=lam_ci_binned))),
                        ...
                        model=c(rep("c",ncuts),rep("ci",ncuts),rep("cc",ncuts),rep("i",ncuts)))
...
l_means<-FIA_lambda %>%
  group_by(PApied_f) %>%
  summarise(lambda_c=mean(lambda_c,na.rm=T), lambda_ci=mean(lambda_ci,na.rm=T),
            lambda_cc=mean(lambda_cc,na.rm=T), lambda_i=mean(lambda_i,na.rm=T))
l_meds<-FIA_lambda %>%
  group_by(PApied_f) %>%
  summarise(lambda_c=median(lambda_c,na.rm=T),lambda_ci=median(lambda_ci,na.rm=T),
            lambda_cc=median(lambda_cc,na.rm=T), lambda_i=median(lambda_i,na.rm=T))

# Niche space plots
FIA_lambda<-na.omit(FIA_lambda)
interpdf_c <-interp2xyz(interp(x=FIA_lambda$PPT_yr, y=FIA_lambda$T_yr, z=FIA_lambda$lambda_c, duplicate="mean"), data.frame=TRUE)
interpdf_ci <-interp2xyz(interp(x=FIA_lambda$PPT_yr, y=FIA_lambda$T_yr, z=FIA_lambda$lambda_ci, duplicate="mean"), data.frame=TRUE)
...
q_neg_ci<-quantile(interpdf_ci$z_diff[which(interpdf_ci$z_diff<0)])
q_pos_ci<-quantile(interpdf_ci$z_diff[which(interpdf_ci$z_diff>0)])
q_ci<-c(q_neg_ci[2:4],q_pos_ci[2:4])
