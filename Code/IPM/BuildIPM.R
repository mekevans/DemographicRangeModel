...
s.x <- function(model, size.x, data, interval = 1, perturb = 0, t.clamp = T) {
  ...
  spred <- 1-predict(model, newdata = scaled.sdata, type = "response", re.form = NA) # no plot rnd effects used by predict here
  
  return(spred + (perturb*spred))
}
...
g.yx <- function(model, growSD, size.y, size.x, data, interval = 1, perturb = 0, t.clamp = F, ba.clamp = T) { # used to be xp, x
...
g.mean <- function(model, size.x, data, interval = 1, perturb = 0, t.clamp = F, ba.clamp = T) {
  # raw (unscaled) data
  gdata <- data.frame(PREVDIA = size.x,
                      BALIVE = ifelse(ba.clamp == T & data$BALIVE > 190, 190, data$BALIVE),
                      #BALIVE = data$BALIVE,
...
fec <- function(model, size.y, size.x, data, interval = 1, perturb = 0, ba.clamp = T) {
  # find scaled value of BALIVE = 2.5 (for clamping)
  # scale(204, scale=r.scaling$scale[3], center=r.scaling$center[3]) # clamp at BALIVE = 204
  
  # raw (unscaled) data
  rdata <- data.frame(BALIVE = ifelse(ba.clamp == T & data$BALIVE > 204, 204, data$BALIVE),
                      #BALIVE = data$BALIVE,
                      T_yr_norm = data$T_yr,
                      PPT_yr_norm = data$PPT_yr)
 ...
 f.mean <- function(model, data, interval = 1, perturb = 0, ba.clamp = T) { 
   # raw (unscaled) data
   rdata <- data.frame(BALIVE = ifelse(ba.clamp == T & data$BALIVE > 204, 204, data$BALIVE),
                       #BALIVE = data$BALIVE,
                       T_yr_norm = data$T_yr,
 ...
 ipm_fun<-function(min, max, n=500, gmodel, smodel, rmodel, gSD,
                   ...
                   s.t.clamp=T, g.t.clamp=F, g.ba.clamp=T,r.ba.clamp=T){
   ...
   # Growth and survival
   G <- h*outer(y, y, g.yx, model = gmodel, growSD = gSD, data = data, perturb = gperturb, t.clamp = g.t.clamp, ba.clamp = g.ba.clamp) # G is an n*n matrix, currently 500*500 = 250,000
   ...
   S <- s.x(model = smodel, y, data = data, interval = 1, perturb = sperturb, t.clamp = s.t.clamp)
   P <- G
   for (k in 1:n) P[,k] <- G[,k]*S #survival*growth subkernel
   
   # Recruitment
   R <- h*outer(y, y, fec, model = rmodel, data = data, perturb = rperturb, ba.clamp = g.ba.clamp)
   # Entire kernel
   K <- P + R
   return(K)
 }
